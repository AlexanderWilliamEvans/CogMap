 ...
 *   `'mouseover'` (default) the layercontrol panel stays expanded while button or panel are hovered.
 *   `'click'` a click on the button toggles the layercontrol visibility.
 * @param {String} opt_options.collapseLabel Text label to use for the expanded layercontrol button. E.g.:
 *   `'»'` (default) or `'\u00BB'`, `'-'` or `'\u2212'`. Not visible if activation mode is `'mouseover'`
 * @param {String} opt_options.label Text label to use for the collapsed layercontrol button. E.g.:
 *   `''` (default), `'«'` or `'\u00AB'`, `'+'`.
...
export default class LayerControl extends Control {
  constructor(opt_options) {
    ...
    this.groupSelectStyle = LayerControl.getGroupSelectStyle(
    ...
    this.hiddenClassName = 'ol-unselectable ol-control layer-control';
    if (LayerControl.isTouchDevice_()) {
    ...
    LayerControl.enableTouchScroll_(this.panel);
    ...
    if (this.activationMode === 'click') {
      // TODO Next: Remove in favour of layer-control-activation-mode-click
  ...
  renderPanel() {
    ...
    LayerControl.renderPanel(this.getMap(), this.panel, {
  ...
  static renderPanel(map, panel, options) {
    ...
    options.groupSelectStyle = LayerControl.getGroupSelectStyle(
      options.groupSelectStyle
    );

    LayerControl.ensureTopVisibleBaseLayerShown_(map);
    ...
    LayerControl.forEachRecursive(map, function (l, idx, a) {
    ...
    if (
      ...
      LayerControl.setGroupVisibility(map);
    } else if (options.groupSelectStyle === 'group') {
      // Set child indetermiate state based on their parent's visibility
      LayerControl.setChildVisibility(map);
    }

    var ul = document.createElement('ul');
    panel.appendChild(ul);
    // passing two map arguments instead of lyr as we're passing the map as the root of the layers tree
    LayerControl.renderLayers_(map, map, ul, options, function render(
      changedLyr
    ) {
      // console.log('render');
      LayerControl.renderPanel(map, panel, options);
    });
  ...
  static setGroupVisibility(map) {
    ...
    const groups = LayerControl.getGroupsAndLayers(map, function (l) {
      return l.getLayers && !l.get('combine') && !LayerControl.isBaseGroup(l);
    }).reverse();
  ...
  static ensureTopVisibleBaseLayerShown_(map) {
    ...
    if (lastVisibleBaseLyr)
      LayerControl.setVisible_(map, lastVisibleBaseLyr, true);
  }
  ...
  static getGroupsAndLayers(lyr, filterFn) {
    ...
    LayerControl.forEachRecursive(lyr, function (l, idx, a) {
      if (l.get('title')) {
        if (filterFn(l, idx, a)) {
          layers.push(l);
        }
  ...
  static setVisible_(map, lyr, visible, groupSelectStyle) {
    ...
    if (
      ...
      lyr.getLayers().forEach((l) => {
        LayerControl.setVisible_(map, l, lyr.getVisible(), groupSelectStyle);
      });
    }
  }
  ...
  static renderLayer_(map, lyr, idx, options, render) {
    ...
    var checkboxId = LayerControl.uuid();

    var label = document.createElement('label');

    if (lyr.getLayers && !lyr.get('combine')) {
      const isBaseGroup = LayerControl.isBaseGroup(lyr);
      ...
      if (lyr.get('fold')) {
        ...
        btn.onclick = function (e) {
          e = e || window.event;
          LayerControl.toggleFold_(lyr, li);
          e.preventDefault();
        };
        li.appendChild(btn);
      }
      ...
      LayerControl.renderLayers_(map, lyr, ul, options, render);
    } else {
      li.className = 'layer';
  ...
  static renderLayers_(map, lyr, elm, options, render) {
    ...
    for (var i = 0, l; i < lyrs.length; i++) {
      l = lyrs[i];
      if (l.get('title')) {
        elm.appendChild(LayerControl.renderLayer_(map, l, i, options, render));
      }
    }
  }
  ...
  static forEachRecursive(lyr, fn) {
    lyr.getLayers().forEach(function (lyr, idx, a) {
      fn(lyr, idx, a);
      if (lyr.getLayers) {
        LayerControl.forEachRecursive(lyr, fn);
      }
    });
  }
...
// Expose LayerControl as ol.control.LayerControl if using a full build of
// OpenLayers
if (window.ol && window.ol.control) {
  window.ol.control.LayerControl = LayerControl;
}